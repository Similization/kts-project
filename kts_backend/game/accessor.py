...
from kts_backend.game.model import PlayerDC, PlayerModel, PlayerGameScoreModel, GameModel, GameDC


class GameAccessor(BaseAccessor):
    @staticmethod
    def convert_player_model_list_to_dc_list(player_model_list: List[PlayerModel]):
        return [
    ...
    @staticmethod
    def convert_game_model_to_dc(game_model: GameModel, players: List[PlayerDC] | List[PlayerModel]) -> GameDC:
        if len(players) != 0 and type(players[0]) is PlayerModel:
            players = GameAccessor.convert_player_model_list_to_dc_list(player_model_list=players)
        return GameDC(
    ...
    async def get_players_by_chat_id(self, chat_id: int) -> List[PlayerDC] | None:
        query = select(PlayerModel) \
            .join(PlayerGameScoreModel) \
            .join(GameModel).where(GameModel.chat_id == chat_id)
        ...
        async with self.app.database.session.begin() as session:
            ...
            if players:
                return self.convert_player_model_list_to_dc_list(player_model_list=players)
            return None

    async def create_new_game(self, players: List[PlayerDC], chat_id: int) -> GameDC:
        ...
        async with self.app.database.session.begin() as session:
            ...
            query = insert(PlayerGameScoreModel).values(game_id=game.game_id) \
    ...
    async def get_players_by_game_id(self, game_id: int) -> List[PlayerDC] | List:
        query = select(PlayerModel).join(PlayerGameScoreModel)\
