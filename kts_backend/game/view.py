...
class GamePlayerListView(View):
    ...
    async def post(self):
        ...
        players: Optional[List[PlayerDC]] = await self.store.game.get_players_by_chat_id(chat_id=chat_id)
        raw_players = [PlayerResponseSchema().dump(player) for player in players]
...
class GameCreateView(View):
    ...
    async def post(self):
        ...
        chat_id = self.data['chat_id']
        game: Optional[GameDC] = await self.store.game.create_new_game(players=players, chat_id=chat_id)
